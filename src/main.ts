...
// 2. GET USER PROJECTS & ASSOCIATED SKILLS
app.get('/api/profile/:id/projects', async (req: Request, res: Response): Promise<void> => {
    const profileId = req.params.id;
...
// Port listener for local testing (Vercel handles ports automatically in the cloud)
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
    console.log(`🚀 Local development backend running on http://localhost:${PORT}`);
});
